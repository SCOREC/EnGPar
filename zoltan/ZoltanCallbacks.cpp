...
namespace zagi {
  ...
  void obj_list(void* data, int, int, ZOLTAN_ID_PTR global_ids,
                ZOLTAN_ID_PTR local_ids, int wgt_dim,float* wgts,int* ierr) {
    agi::Ngraph*  g = static_cast<agi::Ngraph*>(data);

    wgt_dim=0; (void)wgt_dim;
    wgts=NULL; (void)wgts;
  ...
  void hg_data(void* data, int, int, int num_pins, int,
  ...
  void hg_ew(void* data, int, int, int, int,
